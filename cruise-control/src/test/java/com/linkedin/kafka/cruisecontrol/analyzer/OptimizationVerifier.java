...
class OptimizationVerifier {
        ...
        case DEAD_BROKERS:
          ...
          if (!clusterModel.deadBrokers().isEmpty()) {
            ...
            if (resultOfFirstPass != null
                && !verifySoftGoalReplicaMovements(resultOfFirstPass, null, deadBrokers, goalsOfFirstPass)) {
              return false;
            }
            if (resultOfSecondPass != null
                && !verifySoftGoalReplicaMovements(resultOfSecondPass, resultOfFirstPass, deadBrokers, goalsOfSecondPass)) {
              return false;
            }
          }
          break;
  ...
  private static boolean verifySoftGoalReplicaMovements(OptimizerResult result,
    ...
    // execution proposals.
    Map<TopicPartition, Set<Integer>> replicasMovedInPreviousPass = new HashMap<>();
