...
public abstract class AbstractAsyncRequest extends AbstractRequest {
  ...
  public CruiseControlResponse getResponse(CruiseControlRequestContext context)
    ...
    List<OperationFuture>
        futures = _userTaskManager.getOrCreateUserTask(context, this::handle, step, true, parameters());
    ...
    try {
      ...
      LOG.info("Computation is completed for async request: {}.", context.getPathInfo());
    } catch (TimeoutException te) {
      ccResponse = new ProgressResult(futures, _asyncKafkaCruiseControl.config());
      LOG.info("Computation is in progress for async request: {}.", context.getPathInfo());
    }
    return ccResponse;
  }
