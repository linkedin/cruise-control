...
public class LoadMonitor {
  ...
  public LoadMonitorState state() {
    ...
    int numValidSnapshotWindows =
        getAvailableNumSnapshots(_metricSampleAggregator, _metricCompletenessChecker, clusterAndGeneration,
                                 _defaultModelCompletenessRequirements.minMonitoredPartitionsPercentage(),
                                 totalNumPartitions);
    List<Long> loadSnapshotsWindows;
    ...
    if (_metricSampleAggregator.numSnapshotWindows() > _numSnapshots) {
      loadSnapshotsWindows = new ArrayList<>();
      try {
        ...
        if (!loadSnapshots.isEmpty()) {
          for (Snapshot snapshot : loadSnapshots.values().iterator().next()) {
            loadSnapshotsWindows.add(snapshot.time());
          }
        }
        if (loadSnapshotsWindows.isEmpty()) {
          // There are enough snapshot windows but not enough samples yet, fall back to use the snapshot windows
          loadSnapshotsWindows = _metricSampleAggregator.snapshotWindows();
        }
      } catch (Exception e) {
        // let it go
      }
   ...
   * @param modelCompletenessRequirements the load requirements for getting the cluster model.
   * @return A cluster model with the configured number of snapshots whose timestamp is before given timestamp.
   */
  public ClusterModel clusterModel(long now, ModelCompletenessRequirements modelCompletenessRequirements)
      throws ModelInputException, NotEnoughValidSnapshotsException, NotEnoughSnapshotsException {
    ClusterModel clusterModel = clusterModel(-1L, now, modelCompletenessRequirements);
  ...
  public ClusterModel clusterModel(long from, long to, ModelCompletenessRequirements requirements)
    ...
    MetricSampleAggregationResult metricSampleAggregationResult =
        aggregateMetrics(_metricSampleAggregator, _metricCompletenessChecker, from, to, clusterAndGeneration,
                         requirements, totalNumPartitions);
    Map<TopicPartition, Snapshot[]> loadSnapshots = metricSampleAggregationResult.snapshots();
    ...
    int numInvalidPartitions = metricSampleAggregationResult.numInvalidPartitions();
  ...
  public boolean meetLoadRequirements(ModelCompletenessRequirements requirements) {
    MetadataClient.ClusterAndGeneration clusterAndGeneration = _metadataClient.refreshMetadata();
    int availableNumSnapshots = getAvailableNumSnapshots(_metricSampleAggregator,
                                                         _metricCompletenessChecker,
    ...
    int requiredSnapshot = requirements.requiredNumSnapshotWindows();
   ...
   */
  static MetricSampleAggregationResult aggregateMetrics(MetricSampleAggregator aggregator,
    ...
    int requiredNumSnapshotWindows = requirements.requiredNumSnapshotWindows();
    ...
    while (metricSampleAggregationResult == null) {
      int availableNumSnapshots = getAvailableNumSnapshots(aggregator,
                                                           completenessChecker,
                                                           clusterAndGeneration,
                                                           minMonitoredPartitionsPercentage,
                                                           totalNumPartitions);
      if (availableNumSnapshots < requiredNumSnapshotWindows) {
      ...
      metricSampleAggregationResult =
          aggregator.snapshots(clusterAndGeneration.cluster(),
                               from,
                               to,
   ...
   * The method is package private and static for testing.
   *
   ...
   */
  static int getAvailableNumSnapshots(MetricSampleAggregator aggregator,
                                      MetricCompletenessChecker completenessChecker,
                                      MetadataClient.ClusterAndGeneration clusterAndGeneration,
    ...
    Cluster kafkaCluster = clusterAndGeneration.cluster();
    return completenessChecker.numValidWindows(modelGeneration,
                                               kafkaCluster,
  ...
  private double getMonitoredPartitionsPercentage() {
    ...
    try {
      metricSampleAggregationResult =
          _metricSampleAggregator.snapshots(kafkaCluster, -1, System.currentTimeMillis(), _numSnapshots, false);
    ...
    for (Node node : kafkaCluster.nodes()) {
      totalNumPartitions += kafkaCluster.partitionsForNode(node.id()).size();
    }
    ...
    public void run() {
      try {
        ...
        _numValidSnapshotWindows =
            getAvailableNumSnapshots(_metricSampleAggregator, _metricCompletenessChecker, clusterAndGeneration,
                                     minMonitoredPartitionsPercentage, totalNumPartitions);
