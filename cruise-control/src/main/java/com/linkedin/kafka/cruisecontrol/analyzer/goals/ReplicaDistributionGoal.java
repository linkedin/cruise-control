...
public class ReplicaDistributionGoal extends AbstractGoal {
  ...
  /**
   * Check whether given action is acceptable by this goal. An action is acceptable if the number of replicas at
  ...
  public boolean isActionAcceptable(BalancingAction action, ClusterModel clusterModel) {
    int numLocalReplicas = clusterModel.broker(action.sourceBrokerId()).replicas().size();
    int numRemoteReplicas = clusterModel.broker(action.destinationBrokerId()).replicas().size();
