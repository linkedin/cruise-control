...
public class PostponeUrpExecutionTaskStrategy implements ExecutionTaskStrategy {

  private boolean isDataMoveOnUrp(Cluster cluster, ExecutionTask task) {
  ...
  public Map<Integer, SortedSet<ExecutionTask>> applyStrategy(Collection<ExecutionTask> executionTasks, Cluster cluster) {
    Comparator<ExecutionTask> taskComparator = (ExecutionTask task1, ExecutionTask task2) -> isDataMoveOnUrp(cluster, task1) ?
                                               (isDataMoveOnUrp(cluster, task2) ? (int) (task1.executionId() - task2.executionId()) : 1) :
                                               (isDataMoveOnUrp(cluster, task2) ? -1 : (int) (task1.executionId() - task2.executionId()));
