...
class ReplicaDistributionTarget {
  ...
  private boolean moveReplicaToEligibleBroker(ClusterModel clusterModel,
    ...
    for (int candidateBrokerId : !replicaToMove.isCurrentOffline()
                                 ? sortedCandidateBrokerIds().stream().filter(id -> id != replicaToMove.originalBroker().id())
      ...
      if (!legitMove(replicaToMove, clusterModel.broker(candidateBrokerId), clusterModel, ActionType.INTER_BROKER_REPLICA_MOVEMENT)) {
        continue;
      }
