...
public class ModelCompletenessRequirements {
  private final int _requiredNumSnapshotWindows;
   ...
   * @param requiredNumSnapshotWindows the minimum number of required snapshots to generate the model. Tha value
   *                                   must positive.
   ...
   */
  public ModelCompletenessRequirements(int requiredNumSnapshotWindows,
                                       double minMonitoredPartitionPercentage,
                                       boolean includeAllTopics) {
    if (requiredNumSnapshotWindows <= 0) {
      throw new IllegalArgumentException("Invalid requiredNumSnapshotWindows " + requiredNumSnapshotWindows +
                                             ". The requiredNumSnapshotWindows must be positive.");
    }
    ...
    _requiredNumSnapshotWindows = requiredNumSnapshotWindows;
    _includeAllTopics = includeAllTopics;
    _minMonitoredPartitionsPercentage = minMonitoredPartitionPercentage;
  }

  public int requiredNumSnapshotWindows() {
    return _requiredNumSnapshotWindows;
  }
  ...
  public ModelCompletenessRequirements stronger(ModelCompletenessRequirements other) {
    ...
    return new ModelCompletenessRequirements(Math.max(_requiredNumSnapshotWindows, other.requiredNumSnapshotWindows()),
  ...
  public ModelCompletenessRequirements weaker(ModelCompletenessRequirements other) {
    ...
    return new ModelCompletenessRequirements(Math.min(_requiredNumSnapshotWindows, other.requiredNumSnapshotWindows()),
  ...
  public String toString() {
    return String.format("(requiredNumSnapshots=%d, minMonitoredPartitionPercentage=%.3f, includedAllTopics=%s)",
                         _requiredNumSnapshotWindows, _minMonitoredPartitionsPercentage, _includeAllTopics);
  }
