...
public class KafkaCruiseControl {
   ...
   * @param dryRun whether it is a dry run or not.
   * @param ignoreMinMonitoredPartitions whether ignore the minimum required monitored partitions.
   * @return the optimization result.
  ...
  public GoalOptimizer.OptimizerResult rebalance(List<String> goals,
                                                 boolean dryRun,
                                                 boolean ignoreMinMonitoredPartitions) throws KafkaCruiseControlException {
    ...
    // 2. When the load monitor does not meet the requirement to run the goals and users did not want to ignore the
    //    monitored partitions percentage.
    if ((goals != null && !goals.isEmpty())
        || (!_loadMonitor.meetLoadRequirements(modelCompletenessRequirements) && !ignoreMinMonitoredPartitions)) {
      try (AutoCloseable ignored = _loadMonitor.acquireForModelGeneration()) {
  ...
  public GoalOptimizer.OptimizerResult getOptimizationProposals(List<String> goals,
    ...
    Map<Integer, Goal> goalsByPriority = goalsByPriority(goals);
    try (AutoCloseable ignored = _loadMonitor.acquireForModelGeneration()) {
      ModelCompletenessRequirements requirementsToUse =
          modelCompletenessRequirements(goalsByPriority.values()).weaker(requirements);
      ClusterModel clusterModel = _loadMonitor.clusterModel(-1, System.currentTimeMillis(), requirementsToUse);
      return getOptimizationProposals(clusterModel, goalsByPriority);
    } catch (KafkaCruiseControlException kcce) {
      throw kcce;
    } catch (Exception e) {
      throw new KafkaCruiseControlException(e);
  ...
  public ModelCompletenessRequirements defaultModelCompletenesRequirements() {
    return _goalOptimizer.defaultLoadRequirementOptions();
  }
  ...
  private ModelCompletenessRequirements modelCompletenessRequirements(Collection<Goal> overrides) {
    return overrides == null || overrides.isEmpty() ?
        _goalOptimizer.defaultLoadRequirementOptions() : MonitorUtils.combineLoadRequirementOptions(overrides);
  }
  ...
  private Map<Integer, Goal> goalsByPriority(List<String> goals) {
    ...
    Map<String, Goal> allGoals = AnalyzerUtils.getGoalsMapBySimpleName(_config);
