...
public class IntraBrokerGoalViolationDetector extends BaseGoalViolationDetector implements Runnable {
    ...
    public IntraBrokerGoalViolationDetector(Queue<Anomaly> anomalies, KafkaCruiseControl kafkaCruiseControl,
                ...
                for (Goal goal : _detectionGoals) {
                    if (_kafkaCruiseControl.loadMonitor().meetCompletenessRequirements(goal.clusterModelCompletenessRequirements())) {
                        ...
                        newModelNeeded = optimizeForGoal(clusterModel, goal, goalViolations, excludedBrokersForLeadership,
                                excludedBrokersForReplicaMove, checkPartitionsWithRFGreaterThanNumRacks);
    ...
    protected boolean optimizeForGoal(ClusterModel clusterModel,
        ...
        try {
            ...
            if (checkPartitionsWithRFGreaterThanNumRacks) {
                _hasPartitionsWithRFGreaterThanNumRacks =
                        clusterModel.maxReplicationFactor() > clusterModel.numAliveRacksAllowedReplicaMoves(options);
            }
