...
public class LoadMonitorState {
  ...
  private final List<Long> _numMonitoredSnapshotWindows;
  ...
  private LoadMonitorState(LoadMonitorTaskRunnerState state,
                           ...
                           List<Long> numMonitoredSnapshotWindows,
    ...
    _numMonitoredSnapshotWindows = numMonitoredSnapshotWindows;
  ...
  public String toString() {
    ...
    float monitoringCoverage = (float) _numValidMonitoredPartitions / _totalNumPartitions;
    switch (_loadMonitorTaskRunnerState) {
      ...
      case SAMPLING:
      case PAUSED:
        return String.format("{state: %s%s, MonitoredWindows(%d/%d): %s, NumValidPartitions: %d/%d (%.3f%%), flawedPartitions: %d}",
                             _loadMonitorTaskRunnerState, trained, _numValidSnapshotWindows,
                             _numMonitoredSnapshotWindows.size(), _numMonitoredSnapshotWindows,
                             _numValidMonitoredPartitions, _totalNumPartitions, monitoringCoverage * 100,
                             _sampleFlaws.size());
      case BOOTSTRAPPING:
        return String.format("{state: %s%s, BootstrapProgress: %.3f%%, MonitoredWindows(%d/%d): %s, "
                                 + "NumValidPartitions: %d/%d (%.3f%%), flawedPartitions: %d}",
                             _loadMonitorTaskRunnerState, trained, _bootstrapProgress * 100,
                             _numValidSnapshotWindows, _numMonitoredSnapshotWindows.size(), _numMonitoredSnapshotWindows,
                             _numValidMonitoredPartitions, _totalNumPartitions, monitoringCoverage * 100, _sampleFlaws.size());

      case TRAINING:
        return String.format("{state: %s%s, MonitoredWindows(%d/%d): %s, NumValidPartitions: %d/%d (%.3f%%), flawedPartitions: %d}",
                             _loadMonitorTaskRunnerState, trained, _numMonitoredSnapshotWindows.size(),
                             _numValidSnapshotWindows, _numMonitoredSnapshotWindows, _numValidMonitoredPartitions,
                             _totalNumPartitions, monitoringCoverage * 100, _sampleFlaws.size());

      case LOADING:
        return String.format("{state: %s%s, LoadingProgress: %.3f%%, MonitoredWindows(%d/%d): %s, "
                                 + "NumValidPartitions: %d/%d (%.3f%%), flawedPartitions: %d}",
  ...
  public List<Long> numMonitoredSnapshotWindows() {
    return _numMonitoredSnapshotWindows;
  }
