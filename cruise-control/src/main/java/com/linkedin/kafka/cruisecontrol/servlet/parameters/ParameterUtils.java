...
public final class ParameterUtils {
  ...
  public static final String INTRA_BROKER_REPLICATION_THROTTLE_PARAM = "intra_broker_replication_throttle";
  ...
  static Long intraBrokerReplicationThrottle(CruiseControlRequestContext requestContext, KafkaCruiseControlConfig config,
    ...
    if (value != null && value < 0) {
      throw new UserRequestException(
          String.format("Requested intra-broker replication throttle must be non-negative (Requested: %s).", value));
    }
    return value;
  }

  /**
   * Resolve the intra-broker replication throttle from config, falling back to default.replication.throttle
   * if default.intra.broker.replication.throttle is not explicitly set.
   *
