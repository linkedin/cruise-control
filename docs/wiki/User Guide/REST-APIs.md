...
| intra_broker_replication_throttle             | long      | upper bound on the bandwidth used to move replicas between disks within a broker (in bytes per second). Defaults to default.intra.broker.replication.throttle config, then replication_throttle from this request, then default.replication.throttle config. | null                  | yes       |
